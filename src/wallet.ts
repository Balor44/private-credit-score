...
export async function waitForFundsSafe(
  logger: any,
  wallet: any,
  env: any,
  ...
  logger.info('Wallet address: ' + unshieldedAddress);

  const syncedState = await syncWallet(logger, wallet, timeoutMs);

  const nightTokenRaw = Object.keys(syncedState.unshielded.balances)[0];
  const balance = syncedState.unshielded.balances[nightTokenRaw] ?? 0n;
  logger.info('NIGHT balance: ' + balance);

  if (balance > 0n && syncedState.dust.balance(new Date()) === 0n) {
    logger.info('No dust yet; this can take time to accrue after funding.');
  }
